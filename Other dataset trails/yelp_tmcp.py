...
logging.basicConfig(
    filename='yelp_tmcp_output.log',
    filemode='w',
    format = '%(message)s',
    level=logging.INFO
)

logging.info("Yelp TMCP")
...
logging.info(f"Number of Items (businesses): {num_items}")
...
timestamps = compute_node_timestamps(sorted_reviews, num_nodes)
...
def enhanced_hybrid_matrix(adj_matrix, timestamps):
...
adj_matrix = enhanced_hybrid_matrix(adj_matrix, timestamps)

# %%
num_business = adj_matrix.shape[0]
padding_row = torch.zeros((1, num_business))
adj_matrix = torch.cat([adj_matrix, padding_row], dim=0)
padding_col = torch.zeros((num_business + 1, 1))
...
class TemporalGCNLayer(nn.Module):
    """Graph convolutional layer with temporal embedding and learnable update step."""
...
class SASRec(torch.nn.Module):
    def __init__(self, user_num, item_num, args, adj_matrix, timestamps):
        ...
        self.gcn_layers = nn.ModuleList([
            TemporalGCNLayer(args['hidden_units'], args['hidden_units']).to(self.dev)
...
for epoch in range(1, args['num_epochs'] + 1):
    ...
    if epoch % 20 == 0:
        ...
        logging.info(f'epoch:{epoch}, time: {T:.6f}(s), valid (NDCG@5: {t_valid[0]:.4f}, HR@5: {t_valid[1]:.4f}, MRR: {t_valid[2]:.4f}), '
              f'test (NDCG@5: {t_test[0]:.4f}, HR@5: {t_test[1]:.4f}, MRR: {t_test[2]:.4f})')
...
logging.info(f'Best Validation Metrics: '
      f'NDCG@5: {best_val_ndcg:.4f}, HR@5: {best_val_hr:.4f}, MRR: {best_val_mrr:.4f}')
logging.info(f'Best Test Metrics: '
      f'NDCG@5: {best_test_ndcg:.4f}, HR@5: {best_test_hr:.4f}, MRR: {best_test_mrr:.4f}')
